...
enum {
  TYPE_I, TYPE_U, TYPE_S, TYPE_R, TYPE_J, TYPE_B, TYPE_SFT,
  TYPE_N, // none
};
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = ((sword_t)src1 < (sword_t)imm));
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if(src1 == src2) s->dnpc = s->pc + imm);
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if(src1 != src2) s->dnpc = s->pc + imm);
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, t = src1 + imm, R(dest) = SEXT(BITS(t, 31, 0), 32));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , SFT, R(dest) = (src1 >> imm));
  ...
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(dest) = ((sword_t)src1 < (sword_t)src2));
